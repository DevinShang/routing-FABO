...
void CollectPostOrderSnapshot(const shared_ptr<TreeNode>& node, vector<shared_ptr<TreeNode>>& nodes) {
    if (!node) return;
    for (const auto& child : node->children) CollectPostOrderSnapshot(child, nodes);
...
void Tree::RemovePhyRedundantSteiner() {
    vector<shared_ptr<TreeNode>> nodes;
    if (net) nodes.reserve(net->pins.size() * 3);
    CollectPostOrderSnapshot(source, nodes);
    for (const auto& node : nodes) {
        if (!node->parent || node->loc != node->parent->loc) continue;
        if (node->pin) {
            if (node->parent->pin && node->parent->pin != node->pin) continue;
            node->parent->pin = node->pin;
        }
        for (const auto& c : node->children) TreeNode::SetParent(c, node->parent);
        TreeNode::ResetParent(node);
    }
}
